...
def getIsPlaying(data):
    return data['playing']

def getLikeState(data):
    return data['rating']['liked']

def getDislikeState(data):
    return data['rating']['disliked']

def parseLayout(layout):
    displaystr = ""
    for i in layout:
        if i == 'l':
            displaystr += u'\xf164' if getLikeState(data) else u'\xf087' +  u'\xf165' if getDislikeState(data) else u'\xf088' 
        elif i == 't':
            ...
            displaystr += u'\uf04b' if getIsPlaying(data) else u'\uf04c'
        elif i == '-':
            displaystr += " - "
    return displaystr
